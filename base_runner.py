...
class Runner(object):
    ...
    def __init__(self, config):
        ...
        self.individual_group_si_reward = self.all_args.individual_group_si_reward
        assert self.si_loss_type in [
            "kl",
            "js",
            "bce",
        ], "si_loss_type must be either kl, js, bce"
        TrainingPolicy = not self.no_train_policy
        TrainingSI = not self.no_train_si
        if self.only_use_argmax_policy and (TrainingPolicy or TrainingSI):
            print(
                "WARNING: only_use_argmax_policy is true, so you should not be doing any training."
            )

        self.run_mixed_population = self.all_args.run_mixed_population
        if self.run_mixed_population:
            self.team_a_envs = config.get("teamA_envs", None)
            self.team_b_envs = config.get("teamB_envs", None)
        ...
        if self.no_train_si:
            ...
            if self.train_si_on_joint_team_exp:
                print(
                    "WARNING: Training SI on joint team experience 🟪. This should result in subpar compared to no train."
                )
            else:
                print("Training social influence on individual team policy 🟥🟦. 🚀")
        else:
            print(
                "Training social influence on single policy for step 1 policy runs or step 2 SI runs."
            )
        ...
        if self.all_args.env_name == "SMAC":
            ...
            self.all_args.env_name == "SMACv2"
            or self.all_args.env_name == "StarCraft2v2"
        ):
        ...
        if self.all_args.env_name == "Pursuit":
            num_agents = self.all_args["n_units"]
            num_enemies = self.all_args["n_enemies"]
        ...
        print(
            ...
            SplittingPoint,
            "agents in team A and ",
            num_agents - SplittingPoint,
            "agents in team B",
        )
        ...
        print("share_obs_space: ", self.envs.observation_space)
        ...
        if self.algorithm_name == "mat" or self.algorithm_name == "mat_dec":
            ...
            if self.run_mixed_population:
                ...
                action_space_teamB = self.envs.action_space
                ...
                ObservationSpace_teamB = self.team_b_envs.observation_space
                action_space_teamB = self.team_b_envs.action_space
        ...
        if self.run_mixed_population:
            ...
            assert (
                self.all_args.policy_model_dir_teamA is not None
            ), "You need to provide at least one policy models for mixed population"
        ...
        if self.algorithm_name == "mat" or self.algorithm_name == "mat_dec":
            ...
            if self.run_mixed_population:
                self.trainer_A = TrainAlgo(
                    self.all_args, self.policy_A, device=self.device
                )  # This one for mappo
                self.trainer_B = TrainAlgo(
                    self.all_args, self.policy_B, device=self.device
                )  # This one for mappo
            else:
                self.trainer = TrainAlgo(
        ...
        self.buffer = SharedReplayBuffer(
            ...
            share_observation_space,
            self.envs.action_space,
            run_mixed_population=self.run_mixed_population,
            differentOriginEnvTeams=False,
        )

        self.eval_buffer = SharedReplayBuffer(
            self.all_args,
            self.num_agents,
            self.envs.observation_space,
            share_observation_space_team_B,
        ...
        n_obs = get_shape_from_obs_space(self.envs.observation_space)[0]
        ...
        print(
            "social_influence_net_policy obs_space: ",
            social_influence_observation_space,
        )
        # print("social_influence_net_policy share_obs_space: ", social_influence_share_observation_space)
        print(
            "social_influence_net_policy prediction_space: ",
            social_influence_prediction_space,
        )
        ...
        if params["discrete_actions"]:
            social_influence_n_counterfactuals = (
                int(action_dim) - 2
            )  # because removing the agent's own action and the noops action 0

        if self.run_mixed_population:
            custom_teamA_params = params
            custom_teamB_params = {}
            team_B_action_space = self.team_b_envs.action_space
            # team_B_action_space = self.team_b_envs.action_space
            # if self.differentOriginEnvTeams_B:
            team_B_action_dim = action_dim
            ...
            custom_teamB_params = {
                ...
                # "action_dim": 1, # because discrete action for SMAC
                "discrete_actions": True,
                "n_obs": get_shape_from_obs_space(self.team_b_envs.observation_space)[
                    0
                ],
            }
            ...
            assert (
                self.all_args.si_model_dir_teamA is not None
            ), "You need to provide at least 1 si models for mixed population evaluation"
            self.si_model_dir_teamA = self.all_args.si_model_dir_teamA
            self.si_model_dir_teamB = self.all_args.si_model_dir_teamB
            self.social_influence_network = SocialInfluenceManager(
                num_agents=self.num_agents,
                hidden_size=256,
                device=self.device,
                num_env_steps=self.num_env_steps,
                action_space=action_dim,
                n_obs=n_obs,
                social_influence_n_counterfactuals=social_influence_n_counterfactuals,
                discrete_actions=params["discrete_actions"],
                gifs_dir=self.gifs_dir,
                model_dir_si_fixed=self.si_model_dir_teamA,
                model_dir_si_learning=self.si_model_dir_teamB,
                compare_si_policy=False,
                run_mixed_population=self.run_mixed_population,
                si_loss_type=self.si_loss_type,
                only_use_argmax_policy=self.only_use_argmax_policy,
                ShortHandActions=self.ShortHandActions,
                TwoDifferentTeamEnvs=False,
                TeamBParams=custom_teamB_params,
                TeamAParams=custom_teamA_params,
            )
            assert (
                self.evaluate_two_social_influence == False
            ), "You are doing mixed population so no evaluating two different social influence networks"
            ...
            self.social_influence_network = SocialInfluenceManager(
                ...
                discrete_actions=params["discrete_actions"],
                gifs_dir=self.gifs_dir,
                model_dir_si_fixed=self.model_dir_si_fixed,
                model_dir_si_learning=self.model_dir_si_learning,
                compare_si_policy=self.evaluate_two_social_influence,
                si_loss_type=self.si_loss_type,
                only_use_argmax_policy=self.only_use_argmax_policy,
                ShortHandActions=self.ShortHandActions,
            )
    ...
    def compute(self, group_social_influence, group_social_influence_factor=0.5):
        ...
        if self.run_mixed_population:
            ...
            if self.algorithm_name == "mat" or self.algorithm_name == "mat_dec":
                ...
                self.buffer.social_compute_returns(
                    next_values,
                    self.trainer_A.value_normalizer,
                    group_social_influence=group_social_influence,
                    group_social_influence_factor=group_social_influence_factor,
                )  # Can just use A because this is the same for both
            else:
                rnnStatesCritic = self.buffer.rnn_states_critic_A
                BufferToUse = self.buffer
                next_values = self.trainer_A.policy.get_values(
                    np.concatenate(BufferToUse.share_obs[-1]),
                ...
                next_values = self.trainer_B.policy.get_values(
                    np.concatenate(BufferToUse.share_obs[-1]),
                    np.concatenate(rnnStatesCritic[-1]),
                    np.concatenate(BufferToUse.masks[-1]),
                )
                ...
                BufferToUse.social_compute_returns(
                    next_values,
                    self.trainer_B.value_normalizer,
                    group_social_influence=group_social_influence,
                    group_social_influence_factor=group_social_influence_factor,
                )
    ...
    def eval_compute_group_social_influence_rewards(
        self, Obs_A, actions, Obs_B, AlreadyProcessed=False, episode=0
    ):
        if self.run_mixed_population:
            if AlreadyProcessed:
                ...
                IndividualObs_A, actions_prepped, _ = self.prep_data(
                    Obs_A, actions, rewards=None, true_policies=None, time_adjust=False
                )
                IndividualObs_B, _, _ = self.prep_data(
                    Obs_B, actions, rewards=None, true_policies=None, time_adjust=False
                )
            ...
            group_social_influence_rewards = self.social_influence_network.more_efficient_compute_group_social_influence_rewards_all_types(
                IndividualObs_A,
                actions_prepped,
                TeamBObs=IndividualObs_B,
                chosen_pruning_percentile=self.pruning_amount_for_group_si,
                episode=episode,
            )
            ...
            IndividualObs, IndividualActions, _ = self.prep_data(
                Obs_A, actions, rewards=None, true_policies=None, time_adjust=False
            )
            ## Calculate the social influence rewards
            perAgentSocialInfluenceRewards = (
                self.social_influence_network.compute_group_social_influence_rewards(
                    IndividualObs, IndividualActions
                )
            )
    ...
    def compute_group_social_influence_rewards(self, episode=0):
        ...
        BufferToUse = self.buffer
        IndividualObs, IndividualActions, _ = self.buffer_prep_data(
            time_adjust=False, WhichBufferToUse=BufferToUse
        )
        ## Calculate the social influence rewards
        BufferToUse = self.buffer
        IndividualObs, IndividualActions, _ = self.buffer_prep_data(
        ...
        if self.individual_group_si_reward:
            group_social_influence_rewards = self.social_influence_network.compute_individual_group_social_influence_rewards(
                IndividualObs,
                IndividualActions,
                TeamBObs=IndividualObs,
                chosenRewardType=self.graph_equality_type,
                chosen_pruning_percentile=self.pruning_amount_for_group_si,
            )
        else:
            group_social_influence_rewards = (
                self.social_influence_network.compute_group_social_influence_rewards(
                    IndividualObs,
                    IndividualActions,
                    TeamBObs=IndividualObs,
    ...
    def compute_group_social_influence_rewards_evaluation_single_episode(
        ...
        if self.run_mixed_population:
            metricsA = self.social_influence_network.compute_group_social_influence_rewards_evaluation_single_episode(
                obs,
                actions,
                total_num_steps,
                positionOfAgents,
                minimal_gifs=minimal_gifs,
                frames=frames,
                policies=policies,
                whichNetToUse=self.social_influence_network.social_influence_net_A,
                pruning_amount_for_group_similarity=self.pruning_amount_for_group_si,
                graph_gifs=graph_gifs,
            )
            ...
            metricsB = self.social_influence_network.compute_group_social_influence_rewards_evaluation_single_episode(
                ...
                frames=frames,
                policies=policiesB,
                whichNetToUse=self.social_influence_network.social_influence_net_B,
                pruning_amount_for_group_similarity=self.pruning_amount_for_group_si,
                graph_gifs=False,
            )
            ...
            groupSimilarityMetrics = self.eval_compute_group_social_influence_rewards(
                obs, actions, obsB, AlreadyProcessed=True, episode=total_num_steps
            )

            # add on
            groupSimilarityMetrics = {
                "single_ep_eval/" + k: v for k, v in groupSimilarityMetrics.items()
            }
            ...
            )  # note these are the same policies just padded and not because these are the policies that agents actually use

            metrics.update(comboMetrics)  # adds graph to metrics

        else:

            metrics = self.social_influence_network.compute_group_social_influence_rewards_evaluation_single_episode(
                obs,
                actions,
                total_num_steps,
                positionOfAgents,
    ...
    def prep_data(
        ...
        if (
            time_adjust
        ):  # for calculating losses where you need true policy later, but also no need last obs and action
        ...
        if len(OG_joint_obs.shape) > 3:
            ...
            accounted_for_batch_size = n_rollout_threads * (
                episode_length - 1
            )  # because you have cut off the last experience
            batch_size = n_rollout_threads * episode_length
            if time_adjust:
                batch_size = accounted_for_batch_size

            joint_obs = joint_obs.reshape(
                (batch_size, *OG_joint_obs.shape[2:])
            )  # obs have an extra, so remove the last one
            actions = actions.reshape((batch_size, *OG_actions.shape[2:]))
            if true_policies is not None:
                ListOfTruePoliciesOfAgents = next_policies.reshape(
                    (accounted_for_batch_size, *true_policies.shape[2:])
                )  # always cutting off the last experience
    ...
    def get_loss(self):
        ...
        if self.run_mixed_population:
            ...
            IndividualObs_A, IndividualActions_A, next_policies_A = (
                self.buffer_prep_data(time_adjust=True, WhichBufferToUse=self.buffer)
            )

            train_infos_A = self.social_influence_network.get_loss(
                IndividualObs_A,
                IndividualActions_A,
                next_policies_A,
                AGENTS_CAN_DIE=True,
            ...
            IndividualObs, IndividualActions, next_policies = self.buffer_prep_data(
                time_adjust=True
            )

            train_infos = self.social_influence_network.get_loss(
                IndividualObs, IndividualActions, next_policies, AGENTS_CAN_DIE=True
            )
    ...
    def train_social_influence(self):
        ...
        if self.run_mixed_population:
            ...
            joint_obs_A, actions_A, true_policies_A = self.buffer_prep_data(
                time_adjust=True, WhichBufferToUse=self.buffer
            )

            # if the joint exp requires same policies, I should store same policies
            joint_obs_B, actions_B, true_policies_B = self.buffer_prep_data(
                time_adjust=True, WhichBufferToUse=self.buffer_B
            )

            train_infos = self.social_influence_network.train(
                joint_obs_A,
                actions_A,
                true_policies_A,
                joint_obs_B,
        ...
        else:  # no mixed pop regular training - only one team
            joint_obs, actions, true_policies = self.buffer_prep_data(time_adjust=True)

            train_infos = self.social_influence_network.train(
                joint_obs, actions, true_policies, AGENTS_CAN_DIE=True
    ...
    def _save(self, episode=0, episodeLabel=False, prefix="", whichTrainer=None):
        ...
        if self.algorithm_name == "mat" or self.algorithm_name == "mat_dec":
            ...
            if episodeLabel:  # because eval interval
                ...
                policy_critic = whichTrainer.policy.critic
                torch.save(
                    policy_critic.state_dict(), str(EpisodeSaveDir) + "/critic.pt"
                )
    ...
    def save(self, episode=0, episodeLabel=False):
        ...
        if self.run_mixed_population:
            self._save(
                episode, episodeLabel, prefix="TeamA", whichTrainer=self.trainer_A
            )
            self._save(
                episode, episodeLabel, prefix="TeamB", whichTrainer=self.trainer_B
            )
        else:
        ...
        if self.algorithm_name == "mat" or self.algorithm_name == "mat_dec":
            ...
            if not self.all_args.use_render:
                policy_critic_state_dict = torch.load(
                    os.path.join(model_dir, "critic.pt")
                )
