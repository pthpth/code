...
exppre="AdhocTrain_RandomRewards5050_SMACDiffEnv-MAPPO"
si_train_stage=50 
...
for seed in {30..40..5}
do
    for rewardType in "RandomRewardsUniform" "RandomRewardGaussian" "RandomRewardMimic"
    do 
        si_train_stage=50 # Only using for mimic
        policy_train_stage=50
        
        exp="${exppre}-SI${si_train_stage}M-Policy${policy_train_stage}M-Seed${seed}"
        echo "Testing Change: ⭐ Reward type is ${rewardType}"
        echo "env is ${env}, map is ${map}, algo is ${algo}, exp is ${exp}, seed is ${seed}"
        ...
        --group_social_influence ${rewardType} \
