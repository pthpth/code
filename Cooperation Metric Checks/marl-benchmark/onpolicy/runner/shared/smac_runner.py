...
class SMACRunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                ...
                if self.run_mixed_population:
                    ...
                    if self.differentOriginEnvTeams:
                        TeamABuf, TeamBBuf = self.mixed_team_collect(step)
                        actions = TeamABuf[1] # we only need team A format actions
                    else:
                ...
                if self.run_mixed_population:
                    if self.differentOriginEnvTeams:
                        TeamAInfo = [obs, share_obs, rewards, dones, infos, available_actions]
                        obs_TeamB = allThingsTranslationPadding(obs.copy(), TypeDeclaration="observation")
                        state_TeamB = allThingsTranslationPadding(share_obs.copy(), TypeDeclaration="state")
                        available_actions_TeamB = allThingsTranslationPadding(available_actions.copy(), TypeDeclaration="avail_action")
                        TeamBInfo = [obs_TeamB, state_TeamB, rewards, dones, infos, available_actions_TeamB]

                        ## Tester 
                        # ShapeOfAA = available_actions.shape 
                        # ShapeOfNewAA_TeamB = available_actions_TeamB.shape

                        # print("Shape of available actions: ", ShapeOfAA)
                        # print("Sample available actions: ", available_actions)
                        # print("Shape of new available actions (Padded): ", ShapeOfNewAA_TeamB)
                        # print("Sample new available actions (Padded): ", available_actions)
                        
                        # assert np.array_equal(available_actions, allThingsTranslationChopping(available_actions_TeamB, TypeDeclaration="avail_action")), "A check that the chopping and padding are inverses failed"
                        
                        TeamAInfo.extend(TeamABuf)
                        TeamBInfo.extend(TeamBBuf)
            ...
            if episode % self.log_interval == 0:
                ...
                if self.differentOriginEnvTeams:
                    train_infos['average_timestep_reward'] = self.buffer.rewards.sum() / (self.n_rollout_threads * self.num_agents * self.episode_length)
                    # train_infos['total_reward_teamB'] = self.buffer_B.rewards.sum() / (self.n_rollout_threads * self.num_agents * self.episode_length)
                    
                    ### both buffers have same info
                    # train_infos['average_timestep_reward_teamA_graph_similarity'] = self.buffer.group_social_influence_rewards.sum() / (self.n_rollout_threads * self.num_agents * self.episode_length)
                    # train_infos['average_timestep_reward_teamB_graph_similarity'] = self.buffer_B.group_social_influence_rewards.sum() / (self.n_rollout_threads * self.num_agents * self.episode_length)
                    train_infos['average_timestep_reward_graph_similarity'] = self.buffer_B.group_social_influence_rewards.sum() / (self.n_rollout_threads * self.num_agents * self.episode_length)
                else:
    ...
    def warmup(self):
        ...
        if self.differentOriginEnvTeams:
            self.buffer_B.share_obs[0] = allThingsTranslationPadding(share_obs, TypeDeclaration="state")
            self.buffer_B.obs[0] = allThingsTranslationPadding(obs, TypeDeclaration="observation")
            self.buffer_B.available_actions[0] = allThingsTranslationPadding(available_actions.copy(), TypeDeclaration="avail_action")
    ...
    def eval_warmup(self):
        ...
        if self.differentOriginEnvTeams:
            self.eval_buffer_B.share_obs[0] = allThingsTranslationPadding(share_obs, TypeDeclaration="state")
            self.eval_buffer_B.obs[0] = allThingsTranslationPadding(obs, TypeDeclaration="observation")
            self.eval_buffer_B.available_actions[0] = allThingsTranslationPadding(available_actions.copy(), TypeDeclaration="avail_action")
    ...
    def mixed_team_collect(self, step):
        ...
        if self.differentOriginEnvTeams:
            rnnStates = self.buffer.rnn_states
        ...
        if self.run_mixed_population and self.differentOriginEnvTeams:
            ...
            rnnStates = self.buffer_B.rnn_states # just getting from correct buffer so no a & b
            rnnStatesCritic = self.buffer_B.rnn_states_critic # just getting from correct buffer so no a & b
        else:
        ...
        if self.differentOriginEnvTeams:
            # Single value no need to change shape?
            valuesA, valuesB = self.split_for_mixed_teams(values_A, values_B, returnTwoForBuffer=True)
            actionsA, actionsB = self.split_for_mixed_teams(actions_A, actions_B, returnTwoForBuffer=True)
            action_log_probsA, action_log_probsB = self.split_for_mixed_teams(action_log_probs_A, action_log_probs_B, returnTwoForBuffer=True)
            
            # print("OG Team Sample actions_A:", actions_A[0])
            # print("OG Team Sample actions_B:", actions_B[0])
            # print("OG Team A available actions shape:", self.buffer.available_actions[step].shape)
            # print("OG Team B available actions shape:", self.buffer_B.available_actions[step].shape)
            
            assert np.array_equal(self.buffer.available_actions[step], allThingsTranslationChopping(self.buffer_B.available_actions[step], TypeDeclaration="avail_action")), "A check that the chopping and padding are inverses failed (in mixed team collect)"
            
            # print("OG Team A available actions:", self.buffer.available_actions[step])
            # print("OG Team B available actions:", self.buffer_B.available_actions[step])
            
            
            if self.train_si_on_joint_team_exp:
                true_policyA, true_policyB = self.split_for_mixed_teams(true_policy_A, true_policy_B, returnTwoForBuffer=True, TypeDeclaration="policy")
            else:
                # Store original policy - because this is only used to train SI network
                true_policyA = true_policy_A
                true_policyB = true_policy_B

                    
            # Need to change shape
            # These are the same actions but they are padded differently
            TeamABuf = [valuesA, actionsA, action_log_probsA, rnn_states_A, rnn_states_critic_A, true_policyA]
            TeamBBuf = [valuesB, actionsB, action_log_probsB, rnn_states_B, rnn_states_critic_B, true_policyB]
            
            # assert max(actions_B) <= max(actionsA), f"Max action in team A {max(actionsA)} does not match max action in team B {max(actions_B)}"
            assert max(np.array(actions_B).flatten()) <= 9, f"Max action in team B {max(np.array(actions_B).flatten())}"
            assert max(np.array(actions_A).flatten()) <= 9, f"Max action in team A {max(np.array(actions_A).flatten())}"
            # check that for policy the last value is always zero or eps 
            assert np.all(true_policyB[:, :, -1] < 1e-7), f"Last value in policy B is more than eps {true_policyB[:, :, -1]}"
            
            return TeamABuf, TeamBBuf
    ...
    def split_for_mixed_teams(self, values_A, values_B, returnTwoForBuffer=False, TypeDeclaration=None):
        ...
        if TypeDeclaration is None:
            ...
            values_B = allThingsTranslationChopping(values_B, TypeDeclaration)
    ...
    def two_buffer_insert(self, data):
        assert self.run_mixed_population and self.differentOriginEnvTeams, "This function should only be called when mixed population is on and the teams are from different environments"
    ...
    def log_train(self, train_infos, total_num_steps):
        ...
        train_infos["average_step_metric (game)"] = np.mean(self.buffer.game_rewards)
        train_infos["average_step_metric (graph_similarity)"] = np.mean(self.buffer.group_social_influence_rewards)
        train_infos["average_step-log critic_values"] = np.mean(self.buffer.value_preds)
        train_infos["average_step-log returns"] = np.mean(self.buffer.returns)
        train_infos["average_step-log advantages"] = np.mean(self.buffer.advantages)

        # train_infos["average_step_metric (graph_similarity - std)"] = np.std(self.buffer.group_social_influence_rewards)
        # train_infos["average_step_metric (graph_similarity - max)"] = np.max(self.buffer.group_social_influence_rewards)
        # train_infos["average_step_metric (graph_similarity - min)"] = np.min(self.buffer.group_social_influence_rewards)
        ...
        if self.all_args.group_social_influence == "CombineSocialGameRwd_withPenalty":
            ...
            train_infos["average_step_rewards (weighted random)"] = np.mean(GraphSimilarityRewardsUsed)
        elif self.all_args.group_social_influence == "RandomRewardsUniform":
            # Random Uniform rewards in the shape of the graph similarity rewards from 
            # GraphSimilarityRewards = np.random.uniform(-0.4, 0.4, size=GraphSimilarityRewards.shape)
            GraphSimilarityRewards = np.random.uniform(0.3, 0.85, size=GraphSimilarityRewards.shape)
            ...
            print("WARNING: OnlySocialRwd is not a valid reward for training. You should be doing OnlySocialRwd_withPenalty.")
            GameRewardsUsed = GameRewards * 0 # not used
            GraphSimilarityRewardsUsed = GraphSimilarityRewards
        else:
            GameRewardsUsed = GameRewards
        ...
        train_infos["average_step_rewards (weighted graph_similarity)"] = np.mean(GraphSimilarityRewardsUsed)
    ...
    def eval(self, total_num_steps):
        ...
        eval_available_actions_B = eval_available_actions
        ...
        while True:
            if self.run_mixed_population:
                ...
                self.trainer_B.prep_rollout()
                if self.differentOriginEnvTeams:
                    eval_obs_B = allThingsTranslationPadding(eval_obs, TypeDeclaration="observation")
                    eval_share_obs_B = allThingsTranslationPadding(eval_share_obs, TypeDeclaration="state")
                    eval_available_actions_B = allThingsTranslationPadding(eval_available_actions, TypeDeclaration="avail_action")
            else:
                self.trainer.prep_rollout()
            ...
            if eval_episode >= self.all_args.eval_episodes:
                ...
                if self.use_wandb:
                    ...
                    self.summary_writer.add_scalars("eval_win_rate", metrics, total_num_steps)
                break
        ...
        while not done:
            with torch.no_grad():
                if self.run_mixed_population:
                    ...
                    if self.differentOriginEnvTeams:
                        true_policies_B = allThingsTranslationChopping(true_policies_B, TypeDeclaration="policy")
